...
class DataApp {
  ...
  Future<void> reloadSanPham() async {
    await loadDataProduct().then((value) {
      ...
      logSuccess('Loading dishes information: ${_listProduct.length}');
    });
  }
  ...
  Future<void> reloadCategory() async {
    await loadDataCategory().then((value) {
      ...
      logSuccess('Loading categories: ${_listCategoy.length}');
    });
  }
  ...
  Future<QuerySnapshot<Object?>> loadDataCategory() {
    _listCategoy.clear();
    logInfo('Loading categories ...${_listCategoy.length}');
  ...
  Future<QuerySnapshot<Object?>> loadDataStatusOrder() {
    ...
    logInfo('Loading status orders ...${_listStatusOrder.length}');
  ...
  Future<QuerySnapshot<Object?>> loadDataProduct() {
    _listProduct.clear();
    logInfo('Loading products ...${_listProduct.length}');
  ...
  void initData() async {
    ...
    await loadCart.loadDataCart().then((_) {
      logSuccess('Loading shopping cart information');
    });
   ...
   await reloadCategory();
    ...
    await loadDataStatusOrder().then((value) {
      ...
      logSuccess('Load order status: ${_listStatusOrder.length}');
    });
  ...
  void completeLoad() {
    ...
    try {
      ...
      logSuccess('Loaded successfully');
    } catch (e) {
      logError('Invalid router path');
    }
  }
}
