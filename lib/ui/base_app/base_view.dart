    ...
    return FutureBuilder(
        ...
        builder: (context, snapshot) {
          if (snapshot.connectionState == ConnectionState.waiting) {
            logInfo('$this => Loading data ');
            return getSkeletonView(context, controller);
          } else if (snapshot.hasError) {
            logError('$this => An error occured ');
            return getWarningView('An error occurred => incorrect API');
          } else {
           
            if (snapshot.data!.docs.isEmpty) {
              logError('$this => No data available ');
              return getEmtpyView();
            }
            controller.setData(snapshot.data);
            logSuccess('$this => Data loaded successfully');
            return getMainView(context, controller);
          }
        });
  }
    ...
    return StreamBuilder(
        ...
        builder: (context, snapshot) {
          if (snapshot.connectionState == ConnectionState.waiting) {
            ...
            logError('$this => An error occured ');
            return getWarningView('An error occured => incorrect API');
          } else {
